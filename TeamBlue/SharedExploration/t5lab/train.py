...
def _step_logp_val_ent(actor, step, stencil, key):
    ...
    ent = (-(jax.nn.softmax(mlogits, -1) * jax.nn.log_softmax(mlogits, -1)).sum(-1)
           - (jax.nn.softmax(goal_logits, -1) * jax.nn.log_softmax(goal_logits, -1)).sum(-1)).mean()
    return (glp + mlp).sum(), value.mean(), ent            # (), (), ()
    ...
    for _ep in range(int(t.ppo_epochs)):
        ...
        for mb in jnp.array_split(jax.random.permutation(pk, B), int(t.minibatches)):
            ...
            (loss, logs), grad = grad_fn(
                ...
                t.clip, cfg.loss.vf_coef, cfg.regularization.entropy_coef, lk)
