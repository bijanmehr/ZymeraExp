...
**Architecture:** A small package `fiedler/` inside `zymera_experiments/FiedlerValueEstimation/`. `fiedler.py` computes the ground-truth λ₂ from the zymera comm graph; `features.py` extracts per-agent local features; `datagen.py` rolls out episodes and dumps `(features, adjacency, λ₂)` datasets to `.npz`; `metrics.py` scores predictions; `methods/` holds the two reference estimators; `run_slice.py` ties it together. PyTorch learned methods, the message/size/regularization sweeps, and the balthar harness are a **separate later plan**.
...
**Files:**
- Create: `zymera_experiments/FiedlerValueEstimation/fiedler/__init__.py`
- Create: `zymera_experiments/FiedlerValueEstimation/fiedler/methods/__init__.py`
- Create: `zymera_experiments/FiedlerValueEstimation/.gitignore`
- Create: `zymera_experiments/FiedlerValueEstimation/tests/test_smoke.py`

- [ ] **Step 1: Init git + gitignore** (the folder is not yet a repo)

```bash
cd /Users/bijanmehr/Project.Zymera/zymera_experiments/FiedlerValueEstimation
git init
...
```bash
mkdir -p fiedler/methods tests
touch fiedler/__init__.py fiedler/methods/__init__.py
```
...
Run: `cd /Users/bijanmehr/Project.Zymera/zymera_experiments/FiedlerValueEstimation && python -m pytest tests/test_smoke.py -v`
...
```bash
git add -A && git commit -m "chore: scaffold fiedler package + zymera smoke test"
```
...
Expected: FAIL with `ModuleNotFoundError: No module named 'fiedler.fiedler'`.
...
Expected: FAIL with `ModuleNotFoundError: No module named 'fiedler.features'`.
...
Expected: FAIL with `ModuleNotFoundError: No module named 'fiedler.metrics'`.
...
**Files:**
- Create: `fiedler/config.py`
- Create: `fiedler/datagen.py`
...
Expected: FAIL with `ModuleNotFoundError: No module named 'fiedler.config'`.
...
**Files:**
- Create: `fiedler/methods/power_iteration.py`
...
Expected: FAIL with `ModuleNotFoundError: No module named 'fiedler.methods.power_iteration'`.

- [ ] **Step 3: Implement**

`fiedler/methods/power_iteration.py`:
```python
...
**Files:**
- Create: `fiedler/methods/degree_regression.py`
...
Expected: FAIL with `ModuleNotFoundError: No module named 'fiedler.methods.degree_regression'`.

- [ ] **Step 3: Implement**

`fiedler/methods/degree_regression.py`:
```python
