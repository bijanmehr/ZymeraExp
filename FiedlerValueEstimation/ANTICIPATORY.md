...
## Data (`fiedler/datagen.py` additions)
...
- `fiedler/anticipate.py` — neighbor-prediction head, soft-adjacency roll-forward, anomaly-residual scorer.
- `fiedler/models_eqx.py` — `ForecastGCRN` (prediction head + forecast head + online forward).
- `fiedler/datagen.py` — next-step labels, soft regime, adversary injection.
- `fiedler/train_eqx.py` — multi-task forecast loss + truncated-BPTT online training mode (reuses checkpointing).
- `fiedler/metrics.py` — early-warning ROC + anomaly AUC + lead-time helpers.
